...
for the Payflow SDK. If you purchased your account directly from PayPal, use PayPal or leave blank.', 'paypal-for-woocommerce' ),
        ...
        $this->form_fields = apply_filters( 'my_pp__gatewayfc_form_fields', $this->form_fields );
    }
    ...
    function do_payment( $order, $card_number, $card_exp, $card_csc, $centinelPAResStatus = '', $centinelEnrolled = '', $centinelCavv = '', $centinelEciFlag = '', $centinelXid = '')
	{
		...
		{
            $fc_session_expired = apply_filters( 'my_pp__gatewayfc_session_expired', sprintf(__( 'Sorry, your session has expired. <a href=%s>Return to homepage &rarr;</a>', 'paypal-for-woocommerce' ), '"'.home_url().'"'), $this );
		 ...
		 */
		if(!class_exists('my_pp__gatewayPayPal_PayFlow' ))
		{
		...
		$PayPal = new my_pp__gatewayPayPal_PayFlow($PayPalConfig);
		...
		try
		{
	       ...
	       if (trim(number_format(WC()->cart->total, 2, '.', '')) !== trim(number_format($ITEMAMT,2,'.','') + number_format($tax, 2, '.', '') + number_format($shipping, 2, '.', ''))) {
			...
			{
                $fc_empty_response = apply_filters( 'my_pp__gatewayfc_empty_response', __('Empty PayPal response.', 'paypal-for-woocommerce'), $PayPalResult );
			...
			else
			{
                ...
                $fc_error_display_type = apply_filters( 'my_pp__gatewayfc_dp_error_display_type', $fc_error_display_type, $PayPalResult['RESULT'], $PayPalResult['RESPMSG'], $PayPalResult );
				...
				if($this->error_email_notify)
				{
	                ...
	                $message = apply_filters( 'my_pp__gatewayfc_error_email_notify_msg', $message );
	                $subject = apply_filters( 'my_pp__gatewayfc_error_email_notify_subject', "PayPal Pro Error Notification" );
		...
		{
            $fc_connect_error = apply_filters( 'my_pp__gatewayfc_connect_error', __('Connection error:', 'paypal-for-woocommerce' ) . ': "' . $e->getMessage() . '"', $e  );
	...
	function payment_fields() {
        do_action( 'my_pp__gatewaybefore_fc_payment_fields', $this );
		if ( $this->description ) {
			echo '<p>';
			if ( $this->testmode == 'yes' )
		...
		" );
        do_action( 'my_pp__gatewayafter_fc_payment_fields', $this );
	}
    ...
    public function process_refund( $order_id, $amount = null, $reason = '' ) {
        ...
        do_action( 'my_pp__gatewaybefore_fc_refund', $order_id, $amount, $reason );
        ...
        add_action( 'my_pp__gatewayafter_refund', $PayPalResult, $order, $amount, $reason );
        if(isset($PayPalResult['RESULT']) && ($PayPalResult['RESULT'] == 0 || $PayPalResult['RESULT'] == 126)){
            ...
            $fc_refund_error = apply_filters( 'my_pp__gatewayfc_refund_error', $PayPalResult['RESPMSG'], $PayPalResult );
