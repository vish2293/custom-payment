...
class WC_Gateway_PayPal_Pro_AngellEYE extends WC_Payment_Gateway {
    ...
    function init_form_fields() {
        ...
        $this->form_fields = apply_filters( 'my_pp__gatewaypc_form_fields', $this->form_fields );
    }
    /*
     * Admin Options
     */
    ...
    function payment_fields() {
        ...
        do_action( 'before_my_pp__gatewaypc_payment_fields', $this );
        ?>
    ...
    <?php
        do_action( 'after_my_pp__gatewaypc_payment_fields', $this );
    }
    /**
    ...
    function validate_fields() {
        ...
        do_action( 'before_my_pp__gatewaypro_checkout_validate_fields', $card_type, $card_number, $card_csc, $card_exp_month, $card_exp_year );
        ...
        do_action( 'after_my_pp__gatewaypro_checkout_validate_fields', $card_type, $card_number, $card_csc, $card_exp_month, $card_exp_year );
        return true;
    }
    /**
     * Process the payment
        ...
        if ( $this->enable_3dsecure ) {
            ...
            if ( $_SESSION['Centinel_ErrorNo'] == 0 ) {
                if ( $_SESSION['Centinel_Enrolled'] == 'Y' ) {
                    ...
                    $pc_3d_secure_authentication_unavailable = apply_filters( 'my_pp__gatewaypc_process_payment_authentication_unavailable', __('Authentication unavailable. Please try a different payment method or card.','paypal-for-woocommerce') );
                ...
                $pc_3d_secure_authentication = apply_filters( 'my_pp__gatewaypc_process_payment_authentication', __('Error in 3D secure authentication: ', 'paypal-for-woocommerce') . $_SESSION['Centinel_ErrorNo'], $_SESSION['Centinel_ErrorNo'] );
    ...
    function authorise_3dsecure() {
        ...
        if ( $this->liability_shift ) {
            if ( $_SESSION["Centinel_EciFlag"] == '07' || $_SESSION["Centinel_EciFlag"] == '01' ) {
                $pc_authentication_unavailable = apply_filters( 'my_pp__gatewaypc_3d_authentication_unavailable', __('Authentication unavailable.  Please try a different payment method or card.','paypal-for-woocommerce'));
        ...
        if ( $_SESSION['Centinel_ErrorNo'] == "0" ) {
            if ( ($_SESSION["Centinel_PAResStatus"] == "Y" || $_SESSION["Centinel_PAResStatus"] == "A" || $_SESSION["Centinel_PAResStatus"] == "U") && $_SESSION['Centinel_SignatureVerification'] == "Y" ) {
                ...
                $pc_payer_authentication = apply_filters( 'my_pp__gatewaypc_3d_payer_authentication', __('Payer Authentication failed.  Please try a different payment method.','paypal-for-woocommerce'));
            ...
            $pc_3d_secure_authentication = apply_filters( 'my_pp__gatewaypc_3d_secure_authentication', __('Error in 3D secure authentication: ', 'paypal-for-woocommerce') . $_SESSION['Centinel_ErrorDesc'], $_SESSION['Centinel_ErrorDesc']);
            wc_add_notice( $pc_3d_secure_authentication, "error" );
            $order->update_status('failed', sprintf(__('3D Secure error: %s', 'paypal-for-woocommerce'), $_SESSION['Centinel_ErrorDesc'] ) );
            wp_redirect( $redirect_url );
            exit;
	...
	function do_payment($order, $card_number, $card_type, $card_exp_month, $card_exp_year, $card_csc, $centinelPAResStatus = '', $centinelEnrolled = '', $centinelCavv = '', $centinelEciFlag = '', $centinelXid = '')
	{
		...
		{
            $pc_session_expired_error = apply_filters( 'my_pp__gatewaypc_session_expired_error', sprintf(__( 'Sorry, your session has expired. <a href=%s>Return to homepage &rarr;</a>', 'paypal-for-woocommerce' ), '"'.home_url().'"') );
       ...
       if (trim(number_format(WC()->cart->total, 2, '.', '')) !== trim(number_format($ITEMAMT,2,'.','') + number_format($tax, 2, '.', '') + number_format($shipping, 2, '.', ''))) {
		...
		{
            $pc_empty_response = apply_filters( 'my_pp__gatewaypc_empty_response', __('Empty PayPal response.', 'paypal-for-woocommerce'), $PayPalResult );
		...
		else
		{
			...
			if($this->error_email_notify)
			{
                ...
                $pc_error_email_message = apply_filters( 'my_pp__gatewaypc_error_email_notify_message', $message, $error_code, $long_message );
                $pc_error_email_subject = apply_filters( 'my_pp__gatewaypc_error_email_notify_subject', "PayPal Pro Error Notification", $error_code, $long_message );
            ...
            $pc_display_type_error = apply_filters( 'my_pp__gatewaypc_display_type_error', $pc_display_type_error, $error_code, $long_message );
            $pc_display_type_notice = apply_filters( 'my_pp__gatewaypc_display_type_notice', $pc_display_type_notice, $error_code, $long_message );
    ...
    public function process_refund( $order_id, $amount = null, $reason = '' ) {
        ...
        if($PayPal->APICallSuccessful($PayPalResult['ACK']))
        {
            ...
            $pc_message = apply_filters( 'my_pp__gatewaypc_refund_message', $PayPalResult['L_LONGMESSAGE0'], $PayPalResult['L_ERRORCODE'], $PayPalResult );
